...
target_colors_hex = ["#c9e100", "#bae70e"]  # Основные целевые цвета
nearby_colors_hex = ["#abff61", "#87ff27"]  # Вспомогательные целевые цвета
target_colors_hex_2 = ["#ff00c7", "#f009ad"]  # Цвета розовых элементов
freeze_colors_hex = ["#00ffff", "#31bcf3", "#82dce9", "#55ccdc"]  # Цвета заморозки
...
class AutoClicker:
    def __init__(self, hwnd, target_colors_hex, nearby_colors_hex, target_colors_hex_2, freeze_colors_hex, threshold, logger, target_percentage, collect_freeze):
        self.hwnd = hwnd
    ...
    @staticmethod
    def move_mouse_to_top(hwnd, offset_y=100):
        """
        Перемещает мышку в верхнюю часть окна перед прокруткой.
        """
        app = Application().connect(handle=hwnd)
        window = app.window(handle=hwnd)
        window_rect = window.rectangle()
        win32api.SetCursorPos((window_rect.left + window_rect.width() // 2, window_rect.top + offset_y))  # Центр окна по горизонтали, на высоте offset_y пикселей

    def scroll_window(self, direction="down", lines=3):
        """
        Прокручивает окно вверх или вниз.
        """
        try:
            self.move_mouse_to_top(self.hwnd)  # Перемещаем мышь в верхнюю часть окна перед прокруткой
            ...
            self.logger.log(f"Ошибка при прокрутке окна: {e}")
    ...
    def check_and_click_play_button(self, sct, monitor):
        """
        Проверяет наличие кнопки "Играть" и нажимает на неё. Если кнопка не видна, то прокручивает окно.
        """
        ...
        if current_time - self.last_check_time >= CHECK_INTERVAL:
            ...
            if not found_button:
                # Если кнопка не найдена, прокрутить окно вниз
                self.scroll_window("down", lines=3)
                self.logger.log("Прокрутил окно вниз, так как кнопка не была найдена.")
...
if __name__ == "__main__":
    ...
    choice = int(input('''Введите номер окна, в котором открыт бот Blum(кстати правильно выбирайте окно, мышкой
внизу можете посмотреть как называется окно, просто у меня например это TelegramDesktop само окно с блюмом):''')) - 1
    ...
    logger = Logger("BARSKZ")
    logger.log("Вас приветствует скрипт - автокликер для игры Blum")
    logger.log('''После запуска мини игры нажимайте клавишу F6 на клавиатуре (или FN+F6 на ноутбуке), и автокликер сам должен найти кнопку play и запустить игру.''')


    threshold = 0.8  # Порог совпадения шаблона

    auto_clicker = AutoClicker(hwnd, target_colors_hex, nearby_colors_hex, target_colors_hex_2, freeze_colors_hex, threshold, logger, target_percentage, collect_freeze)
    try:
